...
function solution1(dartResult) {
  ...
  for (let i = 0; i < dartResult.length; i++) {
    if (
      parseInt(dartResult[i], 10) === 0 &&
      parseInt(dartResult[i - 1], 10) !== 1
    ) {
      score = parseInt(dartResult[i], 10);
      console.log('숫자 및 자리수 판독', score);
      continue;
    }
    if (parseInt(dartResult[i], 10) && parseInt(dartResult[i + 1], 10) !== 0) {
      score = parseInt(dartResult[i], 10);
      console.log('숫자 및 자리수 판독', score);
      continue;
    }
    if (
      parseInt(dartResult[i], 10) === 1 &&
      parseInt(dartResult[i + 1], 10) === 0
    ) {
      score = 10;
      console.log('숫자 및 자리수 판독', score);
      continue;
